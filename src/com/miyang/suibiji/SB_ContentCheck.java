...
public class SB_ContentCheck extends Activity{
	...
	String photo1,photo2,photo3,actname,search_txt;
		...
		check_back.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				if(actname.equals("ContentActivity")){
					intent.setClass(SB_ContentCheck.this,SB_Content.class);
				}else{
					if(actname.equals("SearchActivity")){
						intent.putExtra("back_txt", search_txt);
						intent.setClass(SB_ContentCheck.this,SB_Search.class);
					}
				}
				SB_ContentCheck.this.startActivity(intent);
				finish();
			}
		});
		...
		check_photo1.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				if(file.exists()){
					...
					if(!photo1.isEmpty()){
						Toast.makeText(SB_ContentCheck.this, "图片资源或已删除！", Toast.LENGTH_SHORT).show();
					}
				}
			}
		});
...
//	获得ContentActivity传来的数据
	private void getdata(){
		Intent it = getIntent();
		actname = it.getStringExtra("actname");
		search_txt = it.getStringExtra("search_txt");
